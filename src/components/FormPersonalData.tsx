...
export default function FormPersonalData (props: PersonalDataProps) {
  const {cpf, setCpf, invisibleErrorCpf, setInvisibleErrorCpf, birthDate, setBirthDate, deficiency, setDeficiency, appearanceDeficiency, setAppearanceDeficiency, appearanceSwitchDeficiency, setAppearanceSwitchDeficiency, providence, setProvidence, socialName, gender, setGender} = useContext(RegistrationContext)
  ...
  return (
    <div>
      <form onSubmit={FormSubmit}>
        <div id="blocoCpf">
          <Input mask="999.999.999-99" textLabel="CPF" defaultValue={cpf || ''} typeInput="text" idInput="cpf" onChange={setCpf} onBlur={validateCpfReturn} classNameInput={setClassNameInputFunction} required={true}/>
          <div className="text-sm text-red-600"  hidden={invisibleErrorCpf}>
            <div className="flex justify-end -mt-3">
              <svg className=" pt-0.5 h-6 w-6 text-red-500"  width="24" height="24" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor" fill="none" strokeLinecap="round" strokeLinejoin="round">  <path stroke="none" d="M0 0h24v24H0z"/>  <circle cx="12" cy="12" r="9" />  <line x1="12" y1="8" x2="12" y2="12" />  <line x1="12" y1="16" x2="12.01" y2="16" /></svg>
              <p className="pt-1 pl-1">CPF inválido!</p>
            </div>
          </div>
      </div>

        <Input textLabel="Data de Nascimento" typeInput="date" idInput="birthDate" defaultValue={birthDate} onChange={setBirthDate} required={true}/>

        <Select textLabel="Gênero" defaultValue={gender} onChange={setGenderFunction} required={true}>
        ...
        <div hidden={!appearanceDeficiency}>
          <Textarea textLabel="Necessito da seguinte providência:" name="deficienty" valueTextArea={providence} onChange={setProvidence}></Textarea>
        </div>
